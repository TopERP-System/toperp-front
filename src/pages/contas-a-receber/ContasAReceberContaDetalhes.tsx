...
import {
  HistoricoPagamentosConta,
  type LinhaHistoricoFallback,
} from '@/components/financeiro/HistoricoPagamentosConta';
import { Button } from '@/components/ui/button';
import { formatarDataBR, formatarFormaPagamento } from '@/lib/utils';
...
const ContasAReceberContaDetalhes = () => {
  ...
  const historicoPagamentos = useMemo((): LinhaHistoricoFallback[] => {
    if (!conta) return [];
  ...
  return (
    <AppLayout>
      <div className="p-4 sm:p-6 space-y-6">
        ...
        <HistoricoPagamentosConta
          contaId={conta.id}
          titulo="Histórico de Recebimentos / Estornos"
          itens={conta.historico_pagamentos}
          fallback={historicoPagamentos}
          labelAtivo="Recebido"
          permiteEstorno={!conta.relacionamentos?.pedido_numero}
          invalidateQueryKeys={[['pedidos', 'contas-receber'], ['dashboard-receber']]}
        />
      </div>
    </AppLayout>
  );
